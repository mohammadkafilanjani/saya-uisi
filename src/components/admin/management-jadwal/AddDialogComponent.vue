<template>
  <div>
    <q-dialog v-model="data.addJadwalDialog" persistent>
      <q-card style="min-width: 350px">
        <q-form @submit="onSubmit()">
          <q-card-section>
            <div class="text-h6">Tambah Jadwal Kelas</div>
          </q-card-section>
          ...
          <q-card-actions align="right">
            <q-btn flat color="primary" label="Cancel" @click="data.addJadwalDialog = false" />
...
const onSubmit = async () => {
  if (!v$.value.$invalid) {
    loading.value = true
    try {
      const response = await useApiWithAuthorization.post('admin/management-jadwal', {
        ...jadwalForm.value,
        kelas_id: data.addedData.kelas_id,
        waktu_selesai: jadwalForm.value.waktu_akhir
      });
      if (response.status === 201) {
        useNotify('Berhasil menambahkan Jadwal Kelas', 'green');
        data.addJadwalDialog = false;
        emit('successEmit');
      }
