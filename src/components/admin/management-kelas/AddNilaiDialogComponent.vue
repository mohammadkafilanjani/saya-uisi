<template>
  <div>
    <q-dialog @before-show="setModel()" v-model="data.addNilaiMahasiswaKelasDialog" persistent>
      <q-card style="min-width: 350px">
        <q-form @submit="onSubmit()">
          <q-card-section>
            <div class="text-h6">Tambah Nilai</div>
          </q-card-section>
          ...
          <q-card-actions align="right">
            <q-btn flat color="primary" label="Cancel" @click="data.addNilaiMahasiswaKelasDialog = false" />
...
const emit = defineEmits(['successAdd']);
...
const onSubmit = async () => {
  if (!v$.value.$invalid) {
    loading.value = true
    try {
      const response = await useApiWithAuthorization.post('admin/nilai', {
        ...nilaiForm.value,
      ...
      if (response.status === 201) {
        ...
        data.addNilaiMahasiswaKelasDialog = false;
        emit('successAdd');
      }
