<template>
  <div>
    <q-dialog @before-show="setModel()" v-model="data.editKelasDialog" persistent>
      <q-card style="min-width: 350px">
        <q-form @submit="onSubmit()">
          ...
          <q-card-actions align="right">
            <q-btn flat color="primary" label="Cancel" @click="data.editKelasDialog = false" />
...
const emit = defineEmits(['successEditKelas']);
...
const setModel = async () => {
  ...
  try {
    ...
    mataKuliahForm.value.name = data.editedKelas.nama_kelas;
    mataKuliahForm.value.semester = data.editedKelas.semester;
    mataKuliahForm.value.tahun_ajaran = data.editedKelas.tahun_ajaran;
    mataKuliahForm.value.mata_kuliah_select = mataKuliahOptions.value.find((matakuliah: MataKuliahOptions) => matakuliah.id === data.editedKelas.mata_kuliah_id) || {
      nama_mata_kuliah: '',
      id: 0,
      label: '',
      value: 0,
    };
    mataKuliahForm.value.dosen = dosenOptions.value.find((dosen: DosenOptions) => dosen.id === data.editedKelas.dosen_id) || {
      name: '',
      id: 0,
      label: '',
      value: 0,
    ...
    const getJadwal = await useApiWithAuthorization.get(`admin/get-kelas-jadwal/${data.editedKelas.id}/${data.editedKelas.waktu_mulai}/${data.editedKelas.waktu_selesai}`)
    mataKuliahForm.value.waktu_mulai = getJadwal.data.data.waktu_mulai;
    mataKuliahForm.value.waktu_selesai = getJadwal.data.data.waktu_selesai;
    mataKuliahForm.value.ruangan = getJadwal.data.data.ruangan;
    mataKuliahForm.value.hari = data.editedKelas.hari;
...
const onSubmit = async () => {
  if (!v$.value.$invalid) {
    loading.value = true
    try {
      const response = await useApiWithAuthorization.put(`admin/management-kelas/${data.editedKelas.id}`, {
        ...
        hari: mataKuliahForm.value.hari,
        waktu_mulai: mataKuliahForm.value.waktu_mulai,
        waktu_selesai: mataKuliahForm.value.waktu_selesai,
        ruangan: mataKuliahForm.value.ruangan,
      });
      if (response.status === 200) {
        useNotify('Berhasil mengedit Kelas', 'green');
        data.editKelasDialog = false;
        emit('successEditKelas');
      }
